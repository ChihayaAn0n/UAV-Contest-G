...
sys.path.extend(['../'])
from . import tools_tegcn


class Feeder(Dataset):
    def __init__(self, data_path, label_path, data_split,
    ...
    def __getitem__(self, index):
        ...
        tensordata = torch.from_numpy(data_numpy).float()
        data_numpy = tensordata.permute(3, 1, 2, 0) # C,T,V,M
        data_numpy = np.array(data_numpy)

        if self.normalization:
            data_numpy = (data_numpy - self.mean_map) / self.std_map
        if self.random_shift:
            data_numpy = tools_tegcn.random_shift(data_numpy)
        if self.random_choose:
            data_numpy = tools_tegcn.random_choose(data_numpy, self.window_size)
        elif self.window_size > 0:
            data_numpy = tools_tegcn.auto_pading(data_numpy, self.window_size)
        if self.random_move:
            data_numpy = tools_tegcn.random_move(data_numpy)
